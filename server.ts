...
app.post("/api/detect-cards", async (req, res) => {
  try {
    ...
    if (!ai) {
      console.warn("GEMINI_API_KEY is not set. Returning fallback grid detector result.");
      res.json(generateFallbackGridDetection("Gemini API Key missing"));
      return;
    }
...
// Fallback grid detector function (guaranteed NO 'by1 is not defined' error)
